...
function getJSONForOrderFeeFlowTx(account, transactionReceipt) {
  ...
  transactionReceipt.logs.forEach((log, i) => {
    ...
    } else if (logAddress === SynthetixPerpsV2ProxyContractAddress && topic0.toLowerCase() === DelayedOrderSubmittedEvent && import_ethers13.BigNumber.from(log.topics[1]).eq(import_ethers13.BigNumber.from(account))) {
      original.fields.push({
...
function getJSONForExecutionFlowTx(account, transactionReceipt) {
  ...
  transactionReceipt.logs.forEach((log, i) => {
    ...
    let logAddress = log.address.toLowerCase();
    let topic0 = log.topics[0].toLowerCase();
    if (logAddress === SynthetixPerpsV2ProxyContractAddress && topic0.toLowerCase() === PositionModifiedEvent && import_ethers13.BigNumber.from(log.topics[2]).eq(import_ethers13.BigNumber.from(account))) {
      logsFound = true;
      data = JSON.stringify({
        block_num: transactionReceipt.blockNumber,
        tx_hash: transactionReceipt.transactionHash,
...
function validTimeNumber(input) {
  return import_moment.default.utc(input.toString(), "YYYYMMDD", true).isValid();
}
function getCurrentDay(input) {
  if (isNaN(input)) {
    return "";
  }
  const date = import_moment.default.utc(input.toString(), "YYYYMMDD", true);
  if (!date.isValid()) {
    return "";
  }
